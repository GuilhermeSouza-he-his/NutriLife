...
const MyProfile = () => {
    ...
    return userData ? (
        <div className='max-w-lg flex flex-col gap-2 text-sm pt-5'>
            ...
            <div>
                <p className='text-gray-600 underline mt-3'>{t('profile.contactInfo')}</p>
                <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-[#363636]'>
                    ...
                    <p className='font-medium'>{t('profile.fields.address')}:</p>
            ...
            <div>
                <p className='text-[#797979] underline mt-3'> {t('profile.basicInfo')}</p>
                <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-gray-600'>
                    <p className='font-medium'>{t('profile.fields.gender')}:</p>

                    {isEdit
                        ? <select className='max-w-20 bg-gray-50' onChange={(e) => setUserData(prev => ({ ...prev, gender: e.target.value }))} value={userData.gender} >
                            <option value="Not Selected">{t('profile.gender.notSelected')}</option>
                    ...
                    <p className='font-medium'>{t('profile.fields.birthday')}:</p>
            ...
            <div className='mt-10'>
                ...
                {isEdit
                    ? <button onClick={updateUserProfileData} className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all'>{t('profile.buttons.save')}</button>
                    : <button onClick={() => setIsEdit(true)} className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all'> {t('profile.buttons.edit')}</button>
                }
